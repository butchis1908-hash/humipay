...
type Pedido = {
  ...
  telefono: string
  humita_dulce: number
  humita_salada: number
  medio_pago: 'yape' | 'plin' | 'efectivo'
  monto_est: number
  pagado: boolean
...
export default function OrdersTable() {
  ...
  async function deletePedido(p: Pedido) {
    const ok = confirm(
      `¿Eliminar el pedido de "${p.nombre}" (S/ ${Number(p.monto_est || 0).toFixed(
        2
    ...
    if (error) {
      alert('No se pudo eliminar el pedido. Revisa permisos o intenta de nuevo.')
      return
    }
    fetchPedidos()
  }
  ...
  const filtered = useMemo(() => {
    return items.filter((p) => {
      ...
      const okPagado =
        fPagado === 'all' ? true : fPagado === 'si' ? p.pagado : !p.pagado
  ...
  // KPIs pagados vs no pagados
  const paidCount = useMemo(() => filtered.filter((p) => p.pagado).length, [filtered])
  const unpaidCount = useMemo(
    () => Math.max(filtered.length - paidCount, 0),
    [filtered, paidCount]
  )
  const percentPaid = filtered.length
    ? Math.round((paidCount / filtered.length) * 100)
    : 0
    ...
    <div className="bg-white border rounded-2xl p-4 shadow-sm space-y-3">
      ...
      <div className="rounded-xl border p-4">
        ...
        <div className="w-full h-3 rounded-full bg-gray-200 overflow-hidden">
          <div className="h-3 bg-green-600" style={{ width: `${percentPaid}%` }} />
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-sm text-gray-700">
        <div>
          Pedidos: <b>{totals.count}</b> • Dulce: <b>{totals.dulce}</b> • Salada:{' '}
          <b>{totals.salada}</b> • Total: <b>S/ {totals.monto.toFixed(2)}</b>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onExport}
            className="px-3 py-1.5 rounded-lg bg-humi-warm text-white hover:bg-humi-deep"
          >
            Exportar Excel
          </button>
        </div>
                ...
                <td className="p-2">
                  <button
                    ...
                    className={
                      ...
                      (p.pagado ? 'bg-green-600' : 'bg-gray-400')
                    }
            ...
            {filtered.length === 0 && (
              <tr>
                <td className="p-4 text-center text-gray-500" colSpan={10}>
                  {loading ? 'Cargando…' : 'Sin pedidos para mostrar.'}
                </td>
              </tr>
            )}
          </tbody>
